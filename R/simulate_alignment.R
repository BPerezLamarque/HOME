...
function(iter,name_index,name,host_tree,mu,n,seed,N,proportion_variant,simul,model,...){
  ...
  if ((simul[iter]!="indep")&(ksi>0)){
    ...
    invisible(capture.output(plot_simulated_switches(host_tree=host_tree,name=name,index=index,switches=switches,path=path)))
  ...
  save(simulated_likelihood,simulated_mu,seed,file=paste("data/simulation_data_",name,"_",index,".RData",sep=""))
  }
