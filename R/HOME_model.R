...
function(name,name_index,path,path_alignment,nb_cores=1,seed=3,nb_tree=5000,lambda,raref=FALSE,empirical=TRUE,randomize=TRUE,nb_random=10){
  ...
  output <- mclapply(1:length(name_index), prepare_data, mc.cores=nb_cores,name=name,name_index=name_index,path=path,path_alignment=path_alignment)
  ...
  for (index in name_index){output <- global_inference(index=index,name=name,nb_tree=nb_tree,lambda=lambda,nb_cores=nb_cores,raref=raref)}
  
  print("Initial output:")
  output <- mclapply(1:length(name_index), output_results, mc.cores=nb_cores,name=name,name_index=name_index,lambda=lambda,nb_tree=nb_tree,empirical=empirical,randomize=F,raref=raref)
  
  print("Model selection:")
  for (index in name_index){output <- model_selection(index=index,name=name,nb_tree=nb_tree,lambda=lambda,nb_cores=nb_cores,seed=seed,nb_random=nb_random)}
  
  print("Output:")
  output <- mclapply(1:length(name_index), output_results, mc.cores=nb_cores,name=name,name_index=name_index,lambda=lambda,nb_tree=nb_tree,empirical=empirical,randomize=T,raref=raref)
